    ...
    with tab4:
        ...
        with col3:
            detection_method = st.selectbox(
                "감지 방법",
                ["실용적 (권장)", "허깅페이스 AI (최신)", "고급 (향상된 특징 + 스펙트럴)", "자동 (MFCC + 클러스터링)", "간단 (에너지 기반)"],
                index=0,  # 실용적(균형잡힌) 감지기를 기본값으로 설정
                help="실용적: 속도와 정확도의 균형 (1-2분), 허깅페이스: 최신 AI 모델 사용 (정확도 최고), 고급: 높은 정확도 (5-10분), 자동: 기본 성능, 간단: 빠르지만 덜 정확함"
            )
